#!/bin/bash
# Taken from democratic-csi repo and modified to run on my local (modified yq commands)
# https://github.com/democratic-csi/democratic-csi/blob/master/contrib/pv-updater.sh
######## REQUIRMENTS #########
# kubectl
...
reclaimPolicy=$(yq -r '.spec.persistentVolumeReclaimPolicy' "${PV_ORIG_FILE}")

# copy file for editing
cp "${PV_ORIG_FILE}" "${PV_TMP_FILE}"

# pre-process before edit
yq -yi 'del(.metadata.resourceVersion)' "${PV_TMP_FILE}"
